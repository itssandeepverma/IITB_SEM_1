...
# set FLAG=1 for feature selection 
# else Without feature selection 
FLAG=1
...
def read_data():
    ...
    dev_input=dev_input.T
    test_input=test_input.T
    #print("old",train_input.shape)
    #print(train_input[1].shape)
    for i in range(0,90,3):
        new_train[i]= train_input[i]*a+train_input[i+1]*b+train_input[i+2]*c
        new_dev[i] =dev_input[i]*a+dev_input[i+1]*b+dev_input[i+2]*c
        new_test[i] =test_input[i]*a+test_input[i+1]*b+test_input[i+2]*c
    #print("New ",new_train.shape)

    if FLAG==1:
        return new_train.T.to_numpy(), train_target.T.to_numpy(), new_dev.T.to_numpy(), dev_target.T.to_numpy() , new_test.T.to_numpy()
    
    return train_input, train_target.T.to_numpy(), dev_input, dev_target.T.to_numpy() , test_input
