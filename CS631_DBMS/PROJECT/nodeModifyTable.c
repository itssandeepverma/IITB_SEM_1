...
char col_values[2][100]; // gets column values on whcih FD are to be checked
	// for A->B,  col_values[0]= A; col_values[1]=B;

char val1[10][100]; // stores the tuple values that are being inserted;
char tab_name[100];	// stores TABLE_NAME;
int index_A=100,index_B=100; // stores the index(in table) of the column A,B;

// gets the TABLE_NAME from query_string
void *get_tn(){
	int i,j;
	int k;
   ...
   memset(tab_name,'\0',sizeof(tab_name));
   	...
   	// our_qs is query_string, updated in postgresql.c and imported using HEAD.h
	for(i=12;our_qs[i]!='\0';i++){
	        if(our_qs[i]==' ')
	        break;
	    }
	 ...
	 for(j=12;j<i;j++)
	        tab_name[j-12]=our_qs[j];

	 tab_name[j]='\0';
	 // stored TABLE_NAME in tab_name;

}

// Gets the tuple values that are being inserted in val1 char array;
void get_vl(){
	int i=0,j=0,k;
	char str2[100];
...
// returns the index of A,B from the order of the column in relation;
int execq2()
{
    int ret;
    int proc;
    ...
    sprintf(new_query,"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = '%s'",tab_name);
    //printf("%s\n",new_query);
    ret = SPI_exec(new_query,100);
    ...
    if (ret > 0 && SPI_tuptable != NULL)
    {
        ...
        for (j = 0; j < proc; j++)
        {
            ...
            if(strcmp(buf2,col_values[0])==0)
            	index_A=j;

            if(strcmp(buf2,col_values[1])==0)
             	index_B=j;
...
// Checks for FD violations;
int execq()
{
		int ret;
	    int proc;
	    char new_string[1000];
	    char new_string2[1000];
	    ...
	    col_num1=index_A; // index of A , for FD A->B;
	    col_num2=index_B; // index of B

	    SPI_connect();

	    sprintf(new_string,"select * from %s where %s = %s",tab_name,col_values[0],val1[col_num1]);
	    									// for A->B,  col_values[0]= A; col_values[1]=B;

	    //Query to check if the value of A being inserted already exist in reltion;
	    ret = SPI_exec(new_string,100);
	    proc = SPI_processed;
	    ...
	    if(proc > 0)
	    {
	    	sprintf(new_string2,"select %s from %s where %s = %s",col_values[1],tab_name,col_values[0],val1[col_num1]);
	    	//printf("%s\n",new_string2);
	    	int ret2 = SPI_exec(new_string2,100);

	    	// Takes the corresponding B values for a given A value;
	    	int proc2 = SPI_processed;
	    	    /*
	    	     * If some rows were fetched, print them via elog(INFO).
	    	     */
	    	    if (ret2 > 0 && SPI_tuptable != NULL)
	    	    {
	    	        ...
	    	        // if the value of B being inserted is not same as the value in relation, raises an ERROR
	    	        if (strcmp(buf2,val1[col_num2])!=0)
	    	        {// val1[col_num2] = value to be inserted, buf2= value in relation;
	    	        	err();
	    	        }
	    	     }
	    	}
 ...
 void check_for_fd(){
	...
	// if insert query is for "FUNC_DEP, need not to check FDs;
	if(strcmp(tab_name,"func_dep")==0)
		return;



	SPI_connect();
	char check_fd_table[1000];
	//printf("tab_name    %s\n",tab_name);
	sprintf(check_fd_table,"select a,b from func_dep where table_name = '%s';",tab_name);
	//printf("%s\n",check_fd_table);
	int ret = SPI_exec(check_fd_table,100);

	// Checks if there is any FD defined for the table in FUNC_DEP;
	...
	memset(col_values[0],'\0',sizeof(col_values[0]));
	memset(col_values[0],'\0',sizeof(col_values[0]));

	// for each FD defined in FUNC_DEP
	// calls execq (which checks FD violations)
	for (j = 0; j < proc; j++)
	{
		HeapTuple tuple = tuptable->vals[j];
		for (i = 1, buf[0] = 0; i <= tupdesc->natts; i++)
		snprintf(buf + strlen (buf), sizeof(buf) - strlen(buf), " %s%s",
		     ...
		     (i == tupdesc->natts) ? " " : " ");
		  	...
		  	while ((token = strtok_r(rest, " ", &rest)))
		  	  {
		  	  strcpy(col_values[k],token);
		  	  k++;
		  	  }

		      execq();
		      //printf("col values[0] %s\n",col_values[0]);
		      //printf("col values[1] %s\n",col_values[1]);
		}
		   ...
		   bool canSetTag)
{
	...
	// Call to check_for_fd() to check for FD violations;
	check_for_fd();
...
void err()
{
	ereport(ERROR,
		    	        			(errcode(ERRCODE_T_R_SERIALIZATION_FAILURE),
		    	        					 errmsg("Functional Dependency Violated")));
