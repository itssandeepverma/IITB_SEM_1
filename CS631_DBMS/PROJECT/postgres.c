...
pg_rewrite_query(Query *query)
{
	...
	{
		/*o don't rewrite utilities, just dump 'em into result list */
...
execq()
{
    ...
    ret = SPI_exec("select * from foo where a = 2",100);
    ...
    if(proc > 0)
    {
    	int ret2 = SPI_exec("select b from foo where a = 2",100);
...
exec_simple_query(const char *query_string)
{
	...
	/* CS631 start */
	//alsso included a header file #include "../main/HEAD.h"
	strcpy(our_qs,query_string);
